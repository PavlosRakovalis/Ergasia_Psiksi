...
U_wall = 0.45  # W/(m²·K) — ΚΕΝΑΚ Ζώνη Γ'

# Οροφή
A_roof = 145.0
U_roof = 0.40  # W/(m²·K) — ΚΕΝΑΚ Ζώνη Γ'
...
h_g_vent = 2445e3  # J/kg_w, τυπική σταθερή τιμή για λανθάνον φορτίο αερισμού
...
2. ΟΡΟΦΗ (δεύτερη μεγαλύτερη συνεισφορά)
   ...
   ► Βελτίωση: Αύξηση μόνωσης (U: 0.40→0.20), ανοιχτόχρωμη μεμβράνη
     (K: 1.0→0.5), cool roof (ψυχρή οροφή).

3. ΤΟΙΧΟΙ
   - Μέτρια συνεισφορά χάρη στο σχετικά χαμηλό U=0.45.
...
# Header block
rows.append(['Δεδομένα Εργασίας','','Ημερομηνία:','21 Ιουλίου','','','','','','Μηχανικός:','Ρακοβαλής Παύλος (ΑΕΜ 6931)','','','','','','','','','','','','','',''])
...
row = ['', f'ṁ={m_dot_air:.4f} kg/s', '', '', f'Wo={Wo}', '', f'Wi={Wi}', '',
       f'ΔW={Wo-Wi:.4f}, hg={h_g_vent/1000:.0f} kJ/kg', '']
for h in hours:
